...
/*
    const tailwindEffects = {
        ...
        ".tailwind-hero-carousel, .tailwind-products-carousel":class{constructor(t){this.scroller=t.querySelector(".block-content")||t,this.previous=null,this.distance=0;let e=document.createElement("nav");for(let i in e.setAttribute("class","twc-nav"),this.scroller.parentNode.appendChild(e),this.indicatorButtons=[],this.slides=[...this.scroller.querySelectorAll(":scope > a, form")],this.slidesPerPage=Math.round(this.scroller.offsetWidth/this.slides[0].offsetWidth),this.slides)if(i%this.slidesPerPage==0){this.slides[i].addEventListener("click",this.capture.bind(this));let s=document.createElement("button");s.innerHTML=+i+1,this.indicatorButtons.push(s),s.addEventListener("click",this.focus.bind(this,this.slides[i])),e.appendChild(s)}this.nextButton=document.createElement("button"),this.nextButton.innerHTML="Next",this.nextButton.setAttribute("class","twc-next"),this.nextButton.addEventListener("click",this.cycle.bind(this,this.slidesPerPage)),this.scroller.parentNode.appendChild(this.nextButton),this.prevButton=document.createElement("button"),this.prevButton.innerHTML="Previous",this.prevButton.setAttribute("class","twc-prev"),this.prevButton.addEventListener("click",this.cycle.bind(this,-this.slidesPerPage)),this.scroller.parentNode.appendChild(this.prevButton),this.scroller.addEventListener("mousedown",this.startDrag.bind(this)),this.scroller.addEventListener("mousemove",this.handleDrag.bind(this)),this.scroller.addEventListener("mouseup",this.endDrag.bind(this)),this.scroller.addEventListener("scroll",this.manualScroll.bind(this),{passive:!0}),this.redraw(this.slides[0]),this.idleDelay=+t.getAttribute("class").split(" idle-").pop(),this.idleDirection=this.slidesPerPage,this.idleTimeout=this.idleDelay?setInterval(this.idle.bind(this),this.idleDelay):null}startDrag(t){window.matchMedia("(pointer:fine)")&&(this.previous=t.clientX)}handleDrag(t){if(clearTimeout(this.idleTimeout),null!==this.previous){t.preventDefault(t);let e=t.clientX,i=e-this.previous;this.scroller.scrollBy({left:-i}),this.distance+=i,this.previous=e}}endDrag(t){this.previous=null,this.focus(),setTimeout(()=>{this.distance=0},1e3)}manualScroll(t){clearTimeout(this.idleTimeout),this.redraw(this.closest())}idle(){let t=this.scroller.getBoundingClientRect(),e=t.bottom>window.innerHeight||t.top<0,i=this.nextButton.hasAttribute("disabled")&&this.idleDirection>0,s=this.prevButton.hasAttribute("disabled")&&this.idleDirection<0;(i||s)&&(this.idleDirection=-this.idleDirection),e||this.cycle(this.idleDirection)}redraw(t){let e=this.slides.indexOf(t);for(let i in e<=0?this.prevButton.setAttribute("disabled",""):this.prevButton.removeAttribute("disabled"),e>=this.slides.length-this.slidesPerPage?this.nextButton.setAttribute("disabled",""):this.nextButton.removeAttribute("disabled"),this.indicatorButtons)this.indicatorButtons[i].setAttribute("data-active",+i===Math.round(e/this.slidesPerPage))}focus(t){t||(t=this.closest()),t.scrollIntoView({behavior:"smooth",block:"nearest",inline:"start"}),this.redraw(t)}closest(){let t=this.scroller.getBoundingClientRect();for(let e of this.slides){let i=e.getBoundingClientRect(),s=i.left-t.left+i.width/2;if(s>0&&s<t.width)return e}return this.slides[this.slides.length-1]}cycle(t,e){e&&(e.preventDefault(),clearTimeout(this.idleTimeout));let i=this.slides.indexOf(this.closest()),s=Math.max(Math.min(i+t,this.slides.length-1),0);this.focus(this.slides[s])}capture(t){Math.abs(this.distance)>10&&t.preventDefault()}},
